...
getlatlongs() {
  // this.livedata = {}
  this.http.get('http://iotmax.in:5300/deviceinfo').subscribe(res => {
    this.livedata = res;
...
alartnew: any = {};
getalart() {
  this.http.get('http://iotmax.in:5300/alertnew').subscribe(res => {
    this.alartnew = res;
    // localStorage.setItem('productdetails', JSON.stringify(this.productdata));
...
getsetlong;
  ngOnInit() {
    this.http.get('http://iotmax.in:5300/alertnewhome').subscribe(res => {
    ...
    this.http.get('http://iotmax.in:5300/plant').subscribe(res => {
      this.plantports = res;
      // localStorage.setItem('productdetails', JSON.stringify(this.productdata));
    ...
    this.http.get('http://iotmax.in:5300/circlezone').subscribe(res => {
      this.czoneports = res;
      // localStorage.setItem('productdetails', JSON.stringify(this.productdata));
    ...
    if (  this.getsetlong == null){
      ...
      this.mymap1 = L.map('map').setView(latLong, 16);
      L.tileLayer(
      ...
      this.mymap1 = L.map('map').setView(latLong, 15);
      L.tileLayer(
        'https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token=pk.eyJ1Ijoic3VicmF0MDA3IiwiYSI6ImNrYjNyMjJxYjBibnIyem55d2NhcTdzM2IifQ.-NnMzrAAlykYciP4RP9zYQ',
        {
          attribution:
...
////////////////////////////////////////////////////////
  ...
  czoneportChange(event: {
    ...
    this.mymap1.setView(new L.LatLng(parseFloat(event.value.lat), parseFloat(event.value.long)), 19);
    localStorage.setItem('setlat', event.value.lat);
    localStorage.setItem('setlong', event.value.long);
  }
  ...
  manytimetimerun(){
    ...
    this.livedata.forEach(element => {
      ...
      var vals = [element.device, element.lat, element.long,element.icon,element.alerts ,element.treeage];
      ...
      for (var i = 0; i < locations.length; i++) {
        ...
        , {icon: L.icon({
            ...
            shadowAnchor: [4, 62],  // the same for the shadow
            popupAnchor:  [-3, -76] // point from which the popup should open relative to the iconAnchor
          })}).bindPopup("Name  : " + locations[i][0] +",  Alart No. : "+ locations[i][4] + ",  Age  : "+ locations[i][5]).openPopup().addTo(this.mymap1);
  ...
  plantdata() {
	...
	keydata2 = 'name'
  pzonedata() {
    this.http.get('http://iotmax.in:5300/polygonzone').subscribe(res => {
